...
routes.set('/image-optimizer/options/optimize', () => {
    assert(optionsToQueryString({ region: Region.Australia, optimize: Optimize.Low }), 'region=australia&optimize=low');
    assert(optionsToQueryString({ region: Region.Australia, optimize: Optimize.High }), 'region=australia&optimize=high');
    assertThrows(() => optionsToQueryString({ region: Region.Australia, optimize: 'invalid' }));
});
...
routes.set('/image-optimizer/options/dpr', () => {
    assert(optionsToQueryString({ region: Region.Asia, dpr: 1.5 }), 'region=asia&dpr=1.500000');
    assert(optionsToQueryString({ region: Region.Asia, dpr: 10 }), 'region=asia&dpr=10.000000');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, dpr: '1001' }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, dpr: 1001 }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, dpr: 0.5 }), TypeError);
});
routes.set('/image-optimizer/options/frame', () => {
    assert(optionsToQueryString({ region: Region.Asia, frame: 1 }), 'region=asia&frame=1');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, frame: 2 }), TypeError);
});
routes.set('/image-optimizer/options/height', () => {
    assert(optionsToQueryString({ region: Region.Asia, height: 1000 }), 'region=asia&height=1000');
    assert(optionsToQueryString({ region: Region.Asia, height: '10%' }), 'region=asia&height=10.000000p');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, height: '1001' }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, height: 100.5 }), TypeError);
});
routes.set('/image-optimizer/options/level', () => {
    assert(optionsToQueryString({ region: Region.Asia, level: '1.1' }), 'region=asia&level=1.1');
    assert(optionsToQueryString({ region: Region.Asia, level: '5.1' }), 'region=asia&level=5.1');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, level: 5.1 }), TypeError);
...
routes.set('/image-optimizer/options/quality', () => {
    assert(optionsToQueryString({ region: Region.Asia, quality: 1 }), 'region=asia&quality=1');
    assert(optionsToQueryString({ region: Region.Asia, quality: 100 }), 'region=asia&quality=100');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, quality: 1001 }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, quality: 1.5 }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, quality: 0.4 }), TypeError);
});
routes.set('/image-optimizer/options/saturation', () => {
    assert(optionsToQueryString({ region: Region.Asia, saturation: 1 }), 'region=asia&saturation=1.000000');
    assert(optionsToQueryString({ region: Region.Asia, saturation: 100 }), 'region=asia&saturation=100.000000');
    assert(optionsToQueryString({ region: Region.Asia, saturation: -100 }), 'region=asia&saturation=-100.000000');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, saturation: 1001 }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, saturation: -101 }), TypeError);
});
...
routes.set('/image-optimizer/options/viewbox', () => {
    assert(optionsToQueryString({ region: Region.Asia, viewbox: 1 }), 'region=asia&viewbox=1');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, viewbox: 2 }), TypeError);
});
routes.set('/image-optimizer/options/width', () => {
    assert(optionsToQueryString({ region: Region.Asia, width: 1000 }), 'region=asia&width=1000');
    assert(optionsToQueryString({ region: Region.Asia, width: '10%' }), 'region=asia&width=10.000000p');
    assertThrows(() => optionsToQueryString({ region: Region.Asia, width: '1001' }), TypeError);
    assertThrows(() => optionsToQueryString({ region: Region.Asia, width: 100.5 }), TypeError);
});