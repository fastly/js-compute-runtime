...
async function app(event) {
    try {
        ...
        console.log(path)
        console.log(fastly.env.get('FASTLY_SERVICE_VERSION'))
        if (routes.has(path)) {
...
// ObjectStore
{
    ...
    // ObjectStore constructor
    {
        ...
        routes.set("/object-store/constructor/called-as-regular-function", async () => {
            let error = assertThrows(() => {
                ObjectStore()
            }, Error, `Constructor ObjectStore requires 'new'`)
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/constructor/parameter-calls-7.1.17-ToString", async () => {
            ...
            error = assertThrows(() => {
                const store = new ObjectStore(Symbol())
        ...
        routes.set("/object-store/constructor/empty-parameter", async () => {
            let error = assertThrows(() => {
                new ObjectStore()
            }, Error, `ObjectStore constructor: At least 1 argument required, but only 0 passed`)
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/constructor/invalid-name", async () => {
            ...
            for (const character of controlCharacters) {
                ...
                }, Error, `ObjectStore name can not contain control characters (\\u0000-\\u001F)`)
                if (error) { return error }
            }
            ...
            }, Error, `ObjectStore name can not be more than 255 characters`)
            if (error) { return error }

            // empty string not allowed
            error = assertThrows(() => {
                new ObjectStore('')
            }, Error, `ObjectStore name can not be an empty string`)
            if (error) { return error }
            return pass()
        });
    }
    // ObjectStore put method
    {
        ...
        routes.set("/object-store/put/called-as-constructor", async () => {
            ...
            }, Error, `ObjectStore.prototype.put is not a constructor`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/called-unbound", async () => {
            ...
            }, Error, "Method put called on receiver that's not an instance of ObjectStore\n")
            if (error) { return error }
            return pass()
        });
        // https://tc39.es/ecma262/#sec-tostring
        ...
        routes.set("/object-store/put/key-parameter-not-supplied", async () => {
            let error = assertThrows(() => {
                const store = createValidStore()
                store.put()
            }, Error, `put: At least 2 arguments required, but only 0 passed`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-empty-string", async () => {
            let error = assertThrows(() => {
                const store = createValidStore()
                store.put('', '')
            }, Error, `ObjectStore key can not be an empty string`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-1024-character-string", () => {
            let error = assertDoesNotThrow(() => {
                const store = createValidStore()
                store.put('a'.repeat(1024), '')
            })
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-1025-character-string", async () => {
            let error = assertThrows(() => {
                const store = createValidStore()
                store.put('a'.repeat(1025), '')
            }, Error, `ObjectStore key can not be more than 1024 characters`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-containing-newline", async () => {
            let error = assertThrows(() => {
                ...
                store.put('\n', '')
            }, Error, `ObjectStore key can not contain newline character`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-containing-carriage-return", async () => {
            let error = assertThrows(() => {
                let store = createValidStore()
                store.put('\r', '')
            }, Error, `ObjectStore key can not contain carriage return character`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-starting-with-well-known-acme-challenge", async () => {
            let error = assertThrows(() => {
                let store = createValidStore()
                store.put('.well-known/acme-challenge/', '')
            }, Error, `ObjectStore key can not start with .well-known/acme-challenge/`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/put/key-parameter-single-dot", async () => {
            let error = assertThrows(() => {
                let store = createValidStore()
                store.put('.', '')
            }, Error, `ObjectStore key can not be "."`)
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/put/key-parameter-containing-special-characters", async () => {
            ...
            for (const character of specialCharacters) {
                let error = assertThrows(() => {
                    let store = createValidStore()
                    store.put(character, '')
                }, Error, `ObjectStore key can not contain ${character} character`)
                if (error) { return error }
            }
            return pass()
        });
        ...
        routes.set("/object-store/put/value-parameter-not-supplied", async () => {
            let error = assertThrows(() => {
                const store = createValidStore()
                store.put("test")
            }, Error, `put: At least 2 arguments required, but only 1 passed`)
            if (error) { return error }
            return pass()
        });
        // - ReadableStream
        routes.set("/object-store/put/value-parameter-readablestream-empty", async () => {
            ...
            let error = assertThrows(() => {
                ...
                const store = createValidStore()
                store.put('readablestream-empty', stream)
            }, Error, `Streaming into an ObjectStore is not yet supported`)
            if (error) { return error }
            return pass()
        ...
        routes.set("/object-store/put/value-parameter-readablestream-under-30mb", async () => {
            // TODO: remove this when streams are supported
            let error = assertThrows(() => {
                const stream = iteratableToStream(['x'.repeat(100) + 'x'])
                const store = createValidStore()
                store.put('readablestream-under-30mb', stream)
            }, Error, `Streaming into an ObjectStore is not yet supported`)
            if (error) { return error }
            return pass()
            // TODO: uncomment this when streams are supported
            // const stream = iteratableToStream(['x'.repeat(100) + 'x'])
            // const store = createValidStore()
            // let result = store.put('readablestream-under-30mb', stream)
        ...
        routes.set("/object-store/put/value-parameter-readablestream-over-30mb", async () => {
            ...
            let error = assertThrows(() => {
                ...
                const store = createValidStore()
                store.put('readablestream-over-30mb', stream)
            }, Error, `Streaming into an ObjectStore is not yet supported`)
            if (error) { return error }
            return pass()
            // TODO: uncomment this when streams are supported
        ...
        routes.set("/object-store/put/value-parameter-readablestream-locked", async () => {
            ...
            // TODO: uncomment this when streams are supported
            // }, Error, `Can't use a ReadableStream that's locked or has ever been read from or canceled as a Request or Response body.`)
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/put/value-parameter-string-over-30mb", async () => {
            ...
            const store = createValidStore()
            let error = assertThrows(() => {
                let result = store.put('string-over-30mb', string)
                store.put("test")
            }, Error, `ObjectStore value can not be more than 30 Megaytes in size.`)
            if (error) { return error }
            return pass()
        });
    ...
    // ObjectStore lookup method
    {
        routes.set("/object-store/lookup/called-as-constructor", async () => {
            ...
            }, Error, `ObjectStore.prototype.lookup is not a constructor`)
            if (error) { return error }
            return pass()
        });
        routes.set("/object-store/lookup/called-unbound", async () => {
            let error = assertThrows(() => {
                ObjectStore.prototype.lookup.call(undefined, '1')
            }, Error, "Method lookup called on receiver that's not an instance of ObjectStore\n")
            if (error) { return error }
            return pass()
        });
        // https://tc39.es/ecma262/#sec-tostring
        ...
        routes.set("/object-store/lookup/key-parameter-not-supplied", async () => {
            let error = assertThrows(() => {
                const store = createValidStore()
                store.lookup()
            }, Error, `lookup: At least 1 argument required, but only 0 passed`)
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/lookup/key-parameter-1024-character-string", () => {
            let error = assertDoesNotThrow(() => {
                const store = createValidStore()
                store.lookup('a'.repeat(1024))
            })
            if (error) { return error }
            return pass()
        });
        ...
        routes.set("/object-store/lookup/key-parameter-starting-with-well-known-acme-challenge", async () => {
            let error = assertThrows(() => {
                let store = createValidStore()
                store.lookup('.well-known/acme-challenge/')
            }, Error, `ObjectStore key can not start with .well-known/acme-challenge/`)
            if (error) { return error }
            return pass()
        });
...
function createValidStore() {
    return new ObjectStore('test-store')
}
...
function toString(object) {
    return Reflect.toString.call(object);
}
function isArguments(object) {
    return toString(object) === '[object Arguments]'
}
