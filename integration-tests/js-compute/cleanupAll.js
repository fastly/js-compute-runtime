...
function existingListIds(stores, prefix) {
    return stores.filter((store) => store.name?.startsWith(prefix) || store.Name?.startsWith(prefix))
...
async function getServiceIds() {
    ...
    return services.filter(({ Name }) => Name.startsWith(SERVICE_PREFIX))
...
await Promise.all([removeConfigStores(serviceIds), removeKVStores(), removeSecretStores(serviceIds), removeAcls(serviceIds)]);
