...
This Class is deprecated, it has been renamed to [`ConfigStore`](../../../config-store/ConfigStore/ConfigStore.mdx) and can be imported via `import { ConfigStore } from 'fastly:config-store'`

The `get()` method exists on the [`ConfigStore`](../../../config-store/ConfigStore/ConfigStore.mdx) Class.
