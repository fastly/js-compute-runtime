...
With one argument, `Math.hypot()` is equivalent to [`Math.abs()`](./abs). [`Math.hypot.length`](../Function/prototype/length.mdx) is 2, which weakly signals that it's designed to handle at least two parameters.
