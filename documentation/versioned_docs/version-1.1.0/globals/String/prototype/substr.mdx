...
- If `start` is omitted or [`undefined`](../../../globals/undefined.md), it's treated as `0`.
- If `length` is omitted or [`undefined`](../../../globals/undefined.md), or if `start + length >= str.length`, `substr()` extracts characters to the end of the string.
- If `length < 0`, an empty string is returned.
- For both `start` and `length`, [`NaN`](../../../globals/NaN.md) is treated as `0`.
