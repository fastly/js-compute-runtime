...
> **Note:** `BigInt()` can only be called without `new`. Attempting to construct it with `new` throws a [`TypeError`](../../globals/TypeError/TypeError.md).
...
- [`TypeError`](../../globals/TypeError/TypeError.md)
  - : Thrown if at least one of these conditions is met:
    ...
    - After conversion to a primitive, the result is [`undefined`](../undefined.md), `null`, `Symbol`.
- [`SyntaxError`](../SyntaxError/SyntaxError.md)
