...
The **`Math.log1p()`** function returns the natural logarithm (base [E](./E.md)) of `1 + x`, where `x` is the argument.
...
The natural logarithm (base [E](./E.md)) of `x + 1`. If `x` is -1, returns [`-Infinity`](../Number/NEGATIVE_INFINITY.md). If `x < -1`, returns `NaN`.
