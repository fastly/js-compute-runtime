...
`Math.atan2()` is passed separate `x` and `y` arguments, while [`Math.atan()`](./atan.md) is passed the ratio of those two arguments. `Math.atan2(y, x)` differs from `Math.atan(y / x)` in the following cases:
