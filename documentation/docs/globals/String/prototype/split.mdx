...
- `separator` _**optional**_
  - : The pattern describing where each split should occur. Can be a string or an object with a [`Symbol.split`](../../../globals/Symbol/split.md) method — the typical example being a regular expression. If undefined, the original target string is returned wrapped in an array.
- `limit` _**optional**_
...
If `separator` is a regular expression with capturing groups, then each time `separator` matches, the captured groups (including any `undefined` results) are spliced into the output array. This behavior is specified by the regexp's [`Symbol.split`](../../../globals/Symbol/split.md) method.

If `separator` is an object with a [`Symbol.split`](../../../globals/Symbol/split.md) method, that method is called with the target string and `limit` as arguments, and `this` set to the object. Its return value becomes the return value of `split`.
