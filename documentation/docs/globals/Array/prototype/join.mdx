...
The `join` method is accessed internally by [`Array.prototype.toString()`](./toString.md) with no arguments. Overriding `join` of an array instance will override its `toString` behavior as well.
