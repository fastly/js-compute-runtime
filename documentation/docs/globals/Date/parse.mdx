...
doesn't represent a valid date, [`NaN`](../NaN.md) is returned.
...
conjunction with the [`setTime()`](./prototype/setTime.md) method and the
`Date` object.
...
defined by ECMA-262 may or may not result in [`NaN`](../NaN.md), depending on the browser
...
values, it will return [`NaN`](../NaN.md):
