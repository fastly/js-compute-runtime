...
rounding in the description of the [`Number.prototype.toFixed()`](../../../globals/Number/prototype/toFixed.md) method,
which also applies to `toPrecision()`.

If the `precision` argument is omitted, behaves as
[`Number.prototype.toString()`](../../../globals/Number/prototype/toString.md). If the `precision` argument is a
...
- `RangeError`
  - : If `precision` is not between `1` and `100`
    (inclusive), a [`RangeError`](../../../globals/RangeError/RangeError.md) is thrown. Implementations are allowed to
