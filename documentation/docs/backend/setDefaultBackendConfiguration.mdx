...
The **`setDefaultBackendConfiguration()`** allows setting backend configuration defaults that should apply to any newly created dynamic backends via the `new Backend()` constructor.
...
```js
setDefaultBackendConfiguration(defaultConfig)
```
