  ...
  - : An Object which contains all the configuration options to apply to the newly created Backend.
    ...
    - `clientCertificate` _: object_ _**optional**_
      ...
      - `key` _: SecretStoreEntry_
        - The `SecretStoreEntry` to use for the key, created via [`SecretStore.prototype.get`](../../secret-store/SecretStore/prototype/get.mdx) or alteratively via [`SecretStore.fromBytes`](../../secret-store/SecretStore/fromBytes.mdx).
