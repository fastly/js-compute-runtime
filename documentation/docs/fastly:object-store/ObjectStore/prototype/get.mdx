...
The `get()` method requires its `this` value to be a [`ObjectStore`](../ObjectStore.md) object.

If the `this` value does not inherit from `ObjectStore.prototype`, a [`TypeError`](../../../globals/TypeError/TypeError.md) is thrown.
