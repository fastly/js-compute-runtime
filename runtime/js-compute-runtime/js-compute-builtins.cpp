...
bool fetch(JSContext *cx, unsigned argc, Value *vp) {
  ...
  fastly_compute_at_edge_fastly_pending_request_handle_t pending_handle = INVALID_HANDLE;
  {
    ...
    if (streaming) {
      ok = fastly_compute_at_edge_fastly_http_req_send_async_streaming(request_handle.handle, body.handle, &backend_str,
                                                &pending_handle, &err);
    } else {
      ok = fastly_compute_at_edge_fastly_http_req_send_async(request_handle.handle, body.handle, &backend_str,
                                      &pending_handle, &err);
    }

    if (!ok) {
      if (err == FASTLY_COMPUTE_AT_EDGE_FASTLY_ERROR_GENERIC_ERROR || err == FASTLY_COMPUTE_AT_EDGE_FASTLY_ERROR_INVALID_ARGUMENT) {
