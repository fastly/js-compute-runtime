...
// Ensure that the handle types stay in sync with what the host expects.
static_assert(std::is_same_v<AsyncHandle::Handle, uint32_t>);
static_assert(std::is_same_v<HttpBody::Handle, uint32_t>);
static_assert(std::is_same_v<HttpPendingReq::Handle, uint32_t>);
static_assert(std::is_same_v<HttpReq::Handle, uint32_t>);
static_assert(std::is_same_v<HttpResp::Handle, uint32_t>);
static_assert(std::is_same_v<LogEndpoint::Handle, uint32_t>);
static_assert(std::is_same_v<Dict::Handle, uint32_t>);
static_assert(std::is_same_v<ObjectStore::Handle, uint32_t>);
static_assert(std::is_same_v<HttpVersion, uint8_t>);
static_assert(std::is_same_v<typeof(CacheOverrideTag::value), uint8_t>);
static_assert(std::is_same_v<typeof(TlsVersion::value), uint8_t>);
static_assert(std::is_same_v<CacheHandle::Handle, uint32_t>);
static_assert(std::is_same_v<typeof(CacheState::state), uint8_t>);
static_assert(std::is_same_v<typeof(BackendHealth::state), uint8_t>);
...
Result<bool> AsyncHandle::is_ready() const {
  ...
  fastly::fastly_host_error err;
  uint32_t ret;
  if (!convert_result(fastly::async_is_ready(this->handle, &ret), &err)) {
    res.emplace_err(err);
  } else {
    res.emplace(static_cast<bool>(ret));
  }
...
Result<std::optional<uint32_t>> AsyncHandle::select(const std::vector<AsyncHandle> &handles,
  ...
  fastly::fastly_host_error err;
  if (!convert_result(fastly::async_select(reinterpret_cast<AsyncHandle::Handle *>(
                                               const_cast<AsyncHandle *>(handles.data())),
                                           handles.size(), timeout_ms, &ret),
                      &err)) {
...
Result<Void> HttpReq::register_dynamic_backend(std::string_view name, std::string_view target,
  ...
  if (auto &val = config.client_cert) {
    ...
    backend_configuration.client_key = config.client_cert->key.handle;
  }
...
bool get_req_header_names(HttpReq::Handle h, fastly::fastly_world_list_string *ret,
                          fastly::fastly_host_error *err) {
  ...
  ret->ptr = static_cast<fastly::fastly_world_string *>(
      cabi_malloc(header_names.size() * sizeof(fastly::fastly_world_string),
                  alignof(fastly::fastly_world_string)));
  auto *next = ret->ptr;
...
bool get_req_header_values(HttpReq::Handle h, fastly::fastly_world_string *name,
                           fastly::fastly_world_option_list_list_u8 *ret,
                           fastly::fastly_host_error *err) {
...
Result<std::vector<HostString>> HttpReq::get_header_names() {
  return generic_get_header_names<get_req_header_names>(this->handle);
}

Result<std::optional<std::vector<HostBytes>>> HttpReq::get_header_values(std::string_view name) {
  return generic_get_header_values<get_req_header_values>(this->handle, name);
}
...
Result<Void> HttpReq::insert_header(std::string_view name, std::span<uint8_t> value) {
  ...
  fastly::fastly_world_list_u8 val = span_to_list_u8(value);
  fastly::fastly_host_error err;
  if (!convert_result(fastly::req_header_insert(this->handle, reinterpret_cast<char *>(hdr.ptr),
                                                hdr.len, val.ptr, val.len),
                      &err)) {
    res.emplace_err(err);
  }
...
Result<Void> HttpReq::append_header(std::string_view name, std::span<uint8_t> value) {
  ...
  fastly::fastly_world_list_u8 val = span_to_list_u8(value);
  fastly::fastly_host_error err;
  if (!convert_result(fastly::req_header_append(this->handle, reinterpret_cast<char *>(hdr.ptr),
                                                hdr.len, val.ptr, val.len),
                      &err)) {
    res.emplace_err(err);
  }
...
Result<Void> HttpReq::remove_header(std::string_view name) {
  ...
  fastly::fastly_host_error err;
  if (!convert_result(
          fastly::req_header_remove(this->handle, reinterpret_cast<char *>(hdr.ptr), hdr.len),
          &err)) {
    res.emplace_err(err);
  }
...
namespace {
bool get_resp_header_names(HttpResp::Handle h, fastly::fastly_world_list_string *ret,
                           fastly::fastly_host_error *err) {
...
bool get_resp_header_values(HttpResp::Handle h, fastly::fastly_world_string *name,
                            fastly::fastly_world_option_list_list_u8 *ret,
                            fastly::fastly_host_error *err) {
  size_t str_max = LIST_ALLOC_SIZE;
  fastly::fastly_world_list_u8 *strs = static_cast<fastly::fastly_world_list_u8 *>(
      cabi_malloc(str_max * sizeof(fastly::fastly_world_list_u8), 1));
  size_t str_cnt = 0;
  size_t nwritten;
  uint8_t *buf = static_cast<uint8_t *>(cabi_malloc(HOSTCALL_BUFFER_LEN, 1));
  uint32_t cursor = 0;
  ...
  while (true) {
    ...
    for (size_t i = 0; i < nwritten; i++) {
      if (buf[i] != '\0')
        continue;
      if (str_cnt == str_max) {
        strs = static_cast<fastly::fastly_world_list_u8 *>(
            cabi_realloc(strs, str_max * sizeof(fastly::fastly_world_list_u8), 1,
                         (str_max + LIST_ALLOC_SIZE) * sizeof(fastly::fastly_world_list_u8)));
        str_max += LIST_ALLOC_SIZE;
      }
      strs[str_cnt].ptr = static_cast<uint8_t *>(cabi_malloc(i - offset + 1, 1));
      strs[str_cnt].len = i - offset;
  ...
  if (str_cnt > 0) {
    ...
    strs = static_cast<fastly::fastly_world_list_u8 *>(
        cabi_realloc(strs, str_max * sizeof(fastly::fastly_world_list_u8), 1,
                     str_cnt * sizeof(fastly::fastly_world_list_u8)));
  } else {
    ret->is_some = false;
    cabi_free(strs);
  }
...
Result<std::vector<HostString>> HttpResp::get_header_names() {
  return generic_get_header_names<get_resp_header_names>(this->handle);
}

Result<std::optional<std::vector<HostBytes>>> HttpResp::get_header_values(std::string_view name) {
  return generic_get_header_values<get_resp_header_values>(this->handle, name);
}
...
Result<Void> HttpResp::insert_header(std::string_view name, std::span<uint8_t> value) {
  ...
  fastly::fastly_world_list_u8 val = span_to_list_u8(value);
  fastly::fastly_host_error err;
  if (!convert_result(fastly::resp_header_insert(this->handle, reinterpret_cast<char *>(hdr.ptr),
                                                 hdr.len, val.ptr, val.len),
                      &err)) {
    res.emplace_err(err);
  }
...
Result<Void> HttpResp::append_header(std::string_view name, std::span<uint8_t> value) {
  ...
  fastly::fastly_world_list_u8 val = span_to_list_u8(value);
  fastly::fastly_host_error err;
  if (!convert_result(fastly::resp_header_append(this->handle, reinterpret_cast<char *>(hdr.ptr),
                                                 hdr.len, val.ptr, val.len),
                      &err)) {
    res.emplace_err(err);
  }
...
Result<Void> HttpResp::remove_header(std::string_view name) {
  ...
  fastly::fastly_host_error err;
  if (!convert_result(
          fastly::resp_header_remove(this->handle, reinterpret_cast<char *>(hdr.ptr), hdr.len),
          &err)) {
    res.emplace_err(err);
  }
...
Result<std::optional<HttpBody>, APIError> ObjectStorePendingLookup::wait() {
  Result<std::optional<HttpBody>, APIError> res;
...
Result<CacheHandle> CacheHandle::lookup(std::string_view key, const CacheLookupOptions &opts) {
  ...
  // fprintf(stderr, "reinterpret_cast<char *>(key_str.ptr): %s\n", reinterpret_cast<char
  // *>(key_str.ptr));
  fprintf(stderr, "key_str.len: %zu\n", key_str.len);
