...
bool SimpleCache::getOrSetThenHandler(JSContext *cx, JS::HandleObject owner, JS::HandleValue extra,
  ...
  fastly_compute_at_edge_cache_handle_t handle = handleVal.toInt32();
  fastly_compute_at_edge_cache_error_t err;
  JS::RootedValue keyVal(cx);
  if (!JS_GetProperty(cx, extraObj, "key", &keyVal)) {
    if (!fastly_compute_at_edge_cache_transaction_cancel(handle, &err)) {
      HANDLE_ERROR(cx, err);
      return RejectPromiseWithPendingError(cx, promise);
    }
    return RejectPromiseWithPendingError(cx, promise);
  ...
  fastly_world_tuple2_fastly_compute_at_edge_cache_body_handle_fastly_compute_at_edge_cache_handle_t
      ret{.f0 = INVALID_HANDLE, .f1 = INVALID_HANDLE};
  if (!fastly_compute_at_edge_cache_transaction_insert_and_stream_back(handle, &options, &ret,
                                                                       &err)) {
    HANDLE_ERROR(cx, err);
    if (!fastly_compute_at_edge_cache_transaction_cancel(handle, &err)) {
      HANDLE_ERROR(cx, err);
  ...
  fastly_compute_at_edge_cache_get_body_options_t opts;
  if (!fastly_compute_at_edge_cache_get_body(ret.f1, &opts, &bodyHandle.handle, &err)) {
    HANDLE_ERROR(cx, err);
    if (!fastly_compute_at_edge_cache_transaction_cancel(handle, &err)) {
      HANDLE_ERROR(cx, err);
      return RejectPromiseWithPendingError(cx, promise);
...
bool SimpleCache::getOrSet(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  fastly_compute_at_edge_cache_handle_t handle = INVALID_HANDLE;
  if (!fastly_compute_at_edge_cache_transaction_lookup(&key, &options, &handle, &err)) {
    HANDLE_ERROR(cx, err);
    return false;
  }
  ...
  if (!fastly_compute_at_edge_cache_get_state(handle, &state, &err)) {
    if (!fastly_compute_at_edge_cache_transaction_cancel(handle, &err)) {
      HANDLE_ERROR(cx, err);
      return ReturnPromiseRejectedWithPendingError(cx, args);
    }
  ...
  if (state & FASTLY_COMPUTE_AT_EDGE_CACHE_LOOKUP_STATE_USABLE) {
    host_api::HttpBody body;
    fastly_compute_at_edge_cache_get_body_options_t opts;
    if (!fastly_compute_at_edge_cache_get_body(handle, &opts, &body.handle, &err)) {
      if (!fastly_compute_at_edge_cache_transaction_cancel(handle, &err)) {
        HANDLE_ERROR(cx, err);
        return ReturnPromiseRejectedWithPendingError(cx, args);
      }
...
bool SimpleCache::set(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  fastly_compute_at_edge_cache_error_t err;
  fastly_compute_at_edge_cache_body_handle_t body_handle = INVALID_HANDLE;
  if (!fastly_compute_at_edge_cache_insert(&key, &options, &body_handle, &err)) {
    HANDLE_ERROR(cx, err);
    return false;
  }
...
bool SimpleCache::get(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  fastly_compute_at_edge_cache_handle_t handle = INVALID_HANDLE;
  if (!fastly_compute_at_edge_cache_lookup(&key, &options, &handle, &err)) {
    HANDLE_ERROR(cx, err);
    return false;
  }
...
bool SimpleCache::purge(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  if (!fastly_compute_at_edge_purge_surrogate_key(&skey, purge_options, &ret, &err)) {
    HANDLE_ERROR(cx, err);
    return false;
  }
