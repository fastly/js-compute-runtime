...
if [ "${1:-default}" != "--keep-debug-info" ]; then
  wasm-tools strip build-debug/starling.wasm/starling.wasm -d ".debug_(info|loc|ranges|abbrev|line|str)" -o ../../fastly.debug.wasm
else
  cp build-debug/starling.wasm/starling.wasm ../../fastly.debug.wasm
fi
