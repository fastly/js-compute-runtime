...
std::unique_ptr<JsonWebKey> JsonWebKey::parse(JSContext *cx, JS::HandleValue value,
  ...
  if (!value.isObject()) {
    api::throw_error(cx, api::Errors::TypeError, "crypto.subtle.importKey", "keyData",
                     "be a JSONWebKey");
    return nullptr;
  }
  ...
  if (kty != required_kty_value) {
    auto message = fmt::format("crypto.subtle.importkey: The JWK member 'kty' was not '{}'",
                               required_kty_value);
    builtins::web::dom_exception::DOMException::raise(cx, message, "DataError");
    return nullptr;
  }
  ...
  std::vector<std::string> key_ops;
  {
    ...
    if (has_key_ops) {
      ...
      if (!key_ops_is_array) {
        ...
        // Array
        builtins::web::dom_exception::DOMException::raise(
            cx, "crypto.subtle.importkey: The JWK member 'key_ops' was not a sequence",
            "DataError");
        return nullptr;
      }
      uint32_t length;
      ...
      if (length != 0) {
        ...
        for (uint32_t i = 0; i < length; ++i) {
          ...
          if (std::find(key_ops.begin(), key_ops.end(), op) != key_ops.end()) {
            builtins::web::dom_exception::DOMException::raise(cx,
                                               "crypto.subtle.importKey parameter 'keyData': "
  ...
  std::vector<RsaOtherPrimesInfo> oth;
  {
    ...
    if (has_oth) {
      ...
      if (!oth_is_array) {
        ...
        builtins::web::dom_exception::DOMException::raise(
            cx, "crypto.subtle.importkey: The JWK member 'oth' was not a sequence", "DataError");
        return nullptr;
      }
      uint32_t length;
        ...
        for (uint32_t i = 0; i < length; ++i) {
          ...
          if (!info_val.isObject()) {
            api::throw_error(cx, api::Errors::TypeError,
                             "crypto.subtle.importKey parameter 'keyData'", "'oth' list",
                             "be an RsaOtherPrimesInfo object");
            return nullptr;
          }
