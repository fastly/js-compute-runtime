...
JSObject *CryptoAlgorithmRSASSA_PKCS1_v1_5_Import::importKey(JSContext *cx, CryptoKeyFormat format,
  ...
  case CryptoKeyFormat::Spki: {
    ...
    rasn::RSlice spki_data(_spki_data.data(), _spki_data.size());
    rasn::SubjectPublicKeyInfo *spki;
    rasn::SpecString spki_parse_err(nullptr, 0, 0);
    ...
    if (!rasn::spki_is_rsa(spki)) {
      rasn::free_spki(spki);
      DOMException::raise(cx, "algorithm not RSA", "DataError");
    }

    rasn::RSAPublicKey *public_key;
    rasn::SpecString public_key_parse_err(nullptr, 0, 0);
    ...
    if (!rasn::decode_public_key(spki, &public_key, &public_key_parse_err)) {
  ...
  case CryptoKeyFormat::Pkcs8: {
    ...
    rasn::RSlice pkcs8_data(_pkcs8_data.data(), _pkcs8_data.size());
    rasn::PrivateKeyInfo *pkcs8;
    rasn::SpecString pkcs8_parse_err(nullptr, 0, 0);
    if (!rasn::decode_pkcs8(&pkcs8_data, &pkcs8, &pkcs8_parse_err)) {
      // 2.3. If an error occurred while parsing, then throw a DataError.
      DOMException::raise(cx, pkcs8_parse_err, "DataError");
    }
    ...
    // DataError.
    if (!rasn::pkcs8_is_rsa(pkcs8)) {
      rasn::free_pkcs8(pkcs8);
      DOMException::raise(cx, "algorithm not RSA", "DataError");
    }

    rasn::RSAPrivateKey *private_key;
    rasn::SpecString private_key_parse_err(nullptr, 0, 0);
    ...
    // [RFC3447], and exactData set to true.
    if (!rasn::decode_private_key(pkcs8, &private_key, &private_key_parse_err)) {
    ...
    auto modulus = rasn::rsa_privkey_modulus(private_key);
    auto exponent = rasn::rsa_privkey_exponent(private_key);
    auto privateExponent = rasn::rsa_privkey_private_exponent(private_key);
    rasn::free_privkey(private_key);
