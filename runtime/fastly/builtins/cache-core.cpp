...
#include "../../../StarlingMonkey/include/errors.h"
#include "../../../StarlingMonkey/runtime/encode.h"
#include "../common/headers-normalization.h"
  ...
  if (headers_val.isObject() && it.valueIsIterable()) {
    ...
    while (true) {
...

      {
        ...
        auto name_chars = common::normalize_header_name(cx, &normalized_name, fun_name);
        if (!name_chars) {
          return JS::Result<host_api::HttpReq>(JS::Error());
        }
        JS::RootedValue normalized_value(cx, value);
        auto value_chars = common::normalize_header_value(cx, &normalized_value, fun_name);
        if (!value_chars) {
          return JS::Result<host_api::HttpReq>(JS::Error());
        }
    ...
    for (size_t i = 0; i < ids.length(); ++i) {
      ...
      JS::RootedValue normalized_name(cx, key);
      auto name_chars =
          common::normalize_header_name(cx, &normalized_name, "CoreCache.transactionLookup");
      if (!name_chars) {
        return JS::Result<host_api::HttpReq>(JS::Error());
      }
      JS::RootedValue normalized_value(cx, value);
      auto value_chars =
          common::normalize_header_value(cx, &normalized_value, "CoreCache.transactionLookup");
      if (!value_chars) {
        return JS::Result<host_api::HttpReq>(JS::Error());
      }
...
// The JavaScript TransactionUpdateOptions parameter we are parsing should have the below
// interface: interface TransactionUpdateOptions {
//   maxAge: number,
//   vary?: Array<string>,
//   initialAge?: number,
...
JS::Result<host_api::CacheWriteOptions> parseTransactionUpdateOptions(JSContext *cx,
  ...
  if (!length_val.isUndefined()) {
    ...
    if (length < 0 || std::isnan(length) || std::isinf(length)) {
      JS_ReportErrorASCII(cx, "length field is an invalid value, only positive numbers can be "
...
// The JavaScript TransactionInsertOptions parameter we are parsing should have the below
// interface: interface TransactionInsertOptions {
//   maxAge: number,
//   vary?: Array<string>,
//   initialAge?: number,
//   staleWhileRevalidate?: number,
