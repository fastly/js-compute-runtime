...
bool KVStore::put(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  std::optional<std::tuple<const uint8_t *, size_t, mozilla::Maybe<JS::AutoCheckCannotGC>>> metadata_buf;
    ...
    if (NativeStreamSource::stream_is_body(cx, body_obj)) {
      ...
      if (!metadata_val.isUndefined()) {
        if (metadata_val.isString()) {
          ...
          metadata_buf = validate_bytes(cx, metadata_val, "KVStore.put metadata");
          if (!metadata_buf) {
            return ReturnPromiseRejectedWithPendingError(cx, args);
          }
      ...
      if (metadata_buf) std::get<2>(*metadata_buf).reset(); // allow GC after hostcall
      if (auto *err = res.to_err()) {
        HANDLE_ERROR(cx, *err);
        return ReturnPromiseRejectedWithPendingError(cx, args);
      }
